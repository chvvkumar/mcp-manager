...
impl ConfigAdapter for ClaudeDesktopAdapter {
    fn tool_id(&self) -> &str { "claude-desktop" }
    fn display_name(&self) -> &str { "Claude Desktop" }
    ...
    fn config_path(&self) -> PathBuf {
        ...
        { dirs::config_dir().unwrap().join("Claude").join("claude_desktop_config.json") }
        #[cfg(target_os = "macos")]
        { dirs::home_dir().unwrap().join("Library/Application Support/Claude/claude_desktop_config.json") }
        #[cfg(target_os = "linux")]
        { dirs::home_dir().unwrap().join(".config/Claude/claude_desktop_config.json") }
    }
