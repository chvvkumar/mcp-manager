...
impl ConfigAdapter for VscodeAdapter {
    fn tool_id(&self) -> &str { "vscode" }
    fn display_name(&self) -> &str { "VS Code" }
    ...
    fn config_path(&self) -> PathBuf {
        ...
        #[cfg(target_os = "macos")]
        { dirs::home_dir().unwrap().join("Library/Application Support/Code/User/mcp.json") }
        #[cfg(target_os = "linux")]
        { dirs::home_dir().unwrap().join(".config/Code/User/mcp.json") }
    }
