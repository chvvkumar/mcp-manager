...
impl ConfigAdapter for CopilotCliAdapter {
    fn tool_id(&self) -> &str { "copilot-cli" }
    fn display_name(&self) -> &str { "Copilot CLI" }

    fn config_path(&self) -> PathBuf {
        if let Some(ref p) = self.config_path_override {
            return p.clone();
        }
        dirs::home_dir().unwrap().join(".copilot/mcp-config.json")
    }
