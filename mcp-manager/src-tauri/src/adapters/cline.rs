...
impl ConfigAdapter for ClineAdapter {
    fn tool_id(&self) -> &str { "cline" }
    ...
    fn config_path(&self) -> PathBuf {
        ...
        { dirs::config_dir().unwrap().join("Code/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json") }
        #[cfg(target_os = "macos")]
        { dirs::home_dir().unwrap().join("Library/Application Support/Code/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json") }
        #[cfg(target_os = "linux")]
        { dirs::home_dir().unwrap().join(".config/Code/User/globalStorage/saoudrizwan.claude-dev/settings/cline_mcp_settings.json") }
    }
